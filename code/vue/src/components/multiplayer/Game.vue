...
const currentUserTurn = computed(() => {
  ...
  return memoryGameStore.currentPlayer === storeAuth.user.id
})
...
const statusGameMessage = computed(() => {
  switch (props.game.gameStatus) {
    case null:
    case 0:
      return currentUserTurn.value ? 'Your turn' : `Opponent's turn (${opponentName.value})`;
    case 1:
    case 2:
...
<template>
  ...
  <Card class="relative grow mx-4 mt-8 pt-2 pb-4 px-1" :class="cardBgColor">
    ...
    <CardContent class="py-4 px-8">
      ...
      <div>
        <Board :size="game.boardSize" @play="playPieceOfBoard"></Board>
      </div>
    </CardContent>
  </Card>
</template>
