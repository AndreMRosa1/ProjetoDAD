...
io.on("connection", (socket) => {
  ...
  socket.on("startGame", (clientGame, board, callback) => {
    if (!util.checkAuthenticatedUser(socket, callback)) {
      return;
    }
    const roomName = "game_" + clientGame.id;
    //console.log("board recebido pelo vue:");
    //console.log(board);
    //ele recebe o tabuleiro
    const game = gameEngine.initGame(clientGame, board);
    //console.log(game)
  ...
  socket.on("play", (cardData, card, board, gameId, callback) => {
    if (!util.checkAuthenticatedUser(socket, callback)) {
        return;
    }
    ...
    // Executa a jogada
    const playResult = gameEngine.playTurn(game, card, socket.id);

    // Caso haja erro na jogada, retorna o erro ao cliente
    if (playResult !== true) {
        if (callback) {
            callback(playResult);
        }
    ...
    // Verifica se um par foi encontrado
    const lastPair = game.discoveredPairs[game.discoveredPairs.length - 1];
    if (lastPair && lastPair.includes(card.id)) {
        // Atualiza o estado do jogo e informa os jogadores apenas quando um par for encontrado
        io.to(roomName).emit("gameUpdated", game);
    }
  ...
  socket.on("addGame", (gridSize, callback) => {
    if (!util.checkAuthenticatedUser(socket, callback)) {
      return;
    }
    const game = lobby.addGame(socket.data.user, socket.id, gridSize);
    io.to("lobby").emit("lobbyChanged", lobby.getGames());
    if (callback) {
      callback(game);
    }
  ...
  socket.on("joinGame", (id, callback) => {
    ...
    // Emite o evento para redirecionar ambos os jogadores
    //console.log(game.board_id);
    //io.to(game.player1SocketId).emit("startGameClient", { game });
    //socket.emit("startGameClient", { game });
  });
