...
Each group has access to their own namespace on kubernetes but they need to register the images and deployments with the proper reference to the group. That reference has the format **dad-group-21**, where X is the group number.
...
# .env.production
VITE_API_DOMAIN=api-dad-group-21.172.22.21.101.sslip.io
VITE_WS_CONNECTION=ws://ws-dad-group-21.172.22.21.101.sslip.io
```
...
Build the Laravel Image (replace group with your group id - dad-group-21 and the version with the current version - 1.0.0):
...
Push the Laravel Image (replace group with your group id - dad-group-21 and the version with the current version - 1.0.0):

```bash
docker push registry.172.22.21.107.sslip.io/{{group}}/api:v{{version}}
```

Build the Vue Image (replace group with your group id - dad-group-21 and the version with the current version - 1.0.0):

```bash
docker build -t registry.172.22.21.107.sslip.io/{{group}}/web:v{{version}} -f ./deployment/DockerfileVue ./vue
```

Push the Vue Image (replace group with your group id - dad-group-21 and the version with the current version - 1.0.0):

```bash
docker push registry.172.22.21.107.sslip.io/{{group}}/web:v{{version}}
```

Build the Node WebSockets Image (replace group with your group id - dad-group-21 and the version with the current version - 1.0.0):

```bash
docker build -t registry.172.22.21.107.sslip.io/{{group}}/ws:v{{version}} -f ./deployment/DockerfileWS ./websockets
```

Push the Node WebSockets Image (replace group with your group id - dad-group-21 and the version with the current version - 1.0.0):
...
-   VUE: [http://web-dad-group-21-172.22.21.101.sslip.io](http://web-dad-group-21-172.22.21.101.sslip.io)
-   Laravel: [http://api-dad-group-21-172.22.21.101.sslip.io](http://web-dad-group-21-172.22.21.101.sslip.io)
...
# get pod name
kubectl get pods -n dad-group-21 -l app=laravel-app


kubectl -n dad-group-21 exec -it <pod-name> -- php artisan migrate:fresh --seed
